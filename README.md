...
| `display.contextPosition` | `ownLine` \| `projectLine` | `ownLine` | Expanded layout: `projectLine` renders the context bar inline on the project line, between the model badge and the project segment, without its label; the standalone context line disappears and the merged context/usage line falls back to usage only. The inlined bar shares the `model` segment key, so a custom `projectLineOrder` moves it together with the model badge (same semantics as the compact layout) |
...
| `display.compactResetTime` | boolean | false | Compact the reset-time suffix: `resets in 3h 32m` becomes `⏰3h32` (unit separator stripped, minutes unit dropped when hours are present: `45m` stays, `3d 14h` becomes `3d14h`). Replaces the `resets in` wording with a clock glyph, overriding `display.showResetLabel` |
| `display.labelOverrides` | object | `{}` | Override the progress-bar labels with custom text, keyed by `context` / `usage` / `weekly` / `approxRam` (e.g. `{ "context": "Ctx", "usage": "Usg", "weekly": "Wkl" }`). Overridden text also drives the label-column alignment width |
| `display.showBalanceLabel` | boolean | true | Show the external usage snapshot's `balance_label` (e.g. `Ollama Pro`) at the end of the usage line. Set to `false` to never append it |
| `display.showContextTokens` | boolean | false | Append the context token count after the percent value in human-readable form (`135 tk`, `3.4k tk`, `1.23m tk`) |
