...
- `display.contextPosition` option to render the context bar inline on the project line (`projectLine`), between the model badge and the project segment without its label; the standalone context line disappears and the merged context/usage line falls back to usage only (#5). The inlined bar shares the `model` segment key, so a custom `projectLineOrder` moves it together with the model badge (#7).
